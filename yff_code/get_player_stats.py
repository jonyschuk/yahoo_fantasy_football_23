...
week_start = days_in_season_elapsed.days // 7 + 1 #pulls the week number
...
def get_player_stats():
    ...
    final_injury_report.to_csv("prod_data/fantasy_injury_report.csv", index=False, header=False, mode='a')

    a=pd.read_csv('prod_data/fantasy_injury_report.csv')
    ...
    b.to_csv('prod_data/final_injury_data.csv', index=False)
    ...
    player_stats.to_csv("prod_data/fantasy_player_stats.csv", index=False)
    print('player stats successful')
    return None
